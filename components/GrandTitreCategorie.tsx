...
interface GrandTitreProps {
  titre: string;
  lien: string;
  color?: string;
}
const GrandTitreCategorie: React.FC<GrandTitreProps> = ({
  titre,
  lien,
  color,
}) => {
  return (
    <div className="border-b mb-5 flex justify-between text-sm">
      <div
        className={`flex items-center pb-2 pr-2 border-b-2  ${
          color ? `text-[${color}]` : "text-indigo-600 "
        } border-indigo-600 uppercase`}
      >
        <svg
          className="h-6 mr-3"
          version="1.1"
        ...
        <a href="#" className="font-semibold inline-block text-xl">
          {titre}
        </a>
      </div>
      <a
        ...
        href={lien}
        className={`${color ? `text-[${color}]` : "text-[#0076d7]"}`}
      >
        Voir plus
      </a>
    </div>
...
export default GrandTitreCategorie;
