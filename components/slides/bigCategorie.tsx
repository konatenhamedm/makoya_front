...
const BigCategorie: React.FC<BigCategorieProps> = ({
  ...
  const fetchFirstDatas = () => {
    setLoading(true);
    axiosAuthapi
      .get("/sousCategorie/sous_categories/categorie/23CAT10001")
      .then((res) => {
  ...
  const fetchSecondDatas = () => {
    ...
    axiosAuthapi
      .get("/sousCategorie/sous_categories/categorie/23CAT10003")
      .then((res) => {
  ...
  return (
    <div className="flex flex-col w-full pt-9 pb-9 gap-4 md:grid md:grid-cols-1 lg:grid-cols-2">
      <div>
        <Card
          isBlurred
